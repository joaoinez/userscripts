...
(() => {
  ...
  const sortables = {
    ...
    tables: {
      check: (el) =>
        el.nodeName === "TH" && el.matches(".markdown-body table thead th"),
    ...
    "all-activity": {
      check: (el) =>
        $("#dashboard") && el.classList.contains("js-all-activity-header"),
      sort: (el) => {
        const list = $$(
          "div[data-repository-hovercards-enabled]:not(.js-details-container) > div",
        );
        const wrap = list.parentElement;
        initSortList(el, list, { selector: "relative-time", attr: "datetime" });
      ...
      css: {
        unsorted: [".js-all-activity-header"],
        extras: [
    ...
    "recent-activity": {
      ...
      sort: (el) => {
        initSortList(el, $$(".js-recent-activity-container ul li"), {
      ...
      css: {
        unsorted: [".news h2:not(.js-all-activity-header)"],
      },
    },
    // github.com (sidebar repos & teams)
    sidebar: {
      check: (el) =>
        $(".dashboard-sidebar") && el.matches(".dashboard-sidebar h2"),
      sort: (el) =>
        initSortList(
          el,
    ...
    pinned: {
      ...
      css: {
        unsorted: [".js-pinned-items-reorder-container h2"],
      },
    },
    // github.com/:org
    "org-repos": {
      ...
      sort: (el) =>
        initSortList(el, $$(".org-repos li"), {
          selector: "a[itemprop*='name']",
        }),
      css: {
        unsorted: [".ghsc-org-repos-header"],
        tweaks: [
          `form[data-autosearch-results-container='org-repositories'] {
						cursor: default;
					}`,
    ...
    "team-members": {
      ...
      setup: () => {
        const form = $(
          "form[data-autosearch-results-container='team-members']",
        );
        if (form) {
      ...
      css: {
        unsorted: [".ghsc-team-members-header"],
      },
    },
    ...
    "org-projects": {
      ...
      sort: (el) =>
        initSortList(el, $$("#projects-results > div"), { selector: "h4 a" }),
      css: {
        unsorted: [".ghsc-project-header"],
      },
    },
    ...
    "user-repos": {
      ...
      sort: (el) =>
        initSortList(el, $$("#user-repositories-list li"), {
          selector: "a[itemprop*='name']",
        }),
      css: {
        unsorted: [".ghsc-repos-header"],
        tweaks: [
          `form[data-autosearch-results-container='user-repositories-list'] {
						cursor: default;
					}`,
    ...
    "user-tab-follow": {
      ...
      check: (el, loc) =>
        loc.search.indexOf("tab=follow") > -1 && el.matches(".ghsc-follow-nav"),
      sort: (el) => {
        initSortList(
          el,
    ...
    "repo-stars-or-watchers": {
      ...
      sort: (el) =>
        initSortList(el, $$(".follow-list-item"), {
          selector: ".follow-list-name",
        }),
      css: {
        unsorted: [".ghsc-gazer-header"],
      },
    },
  };
  ...
  function addRepoFileHeader() {
    ...
    const $target =
      $header && $("div[role='grid'] .sr-only", $header.parentElement);
    if ($header && $target) {
      $target.className =
        "Box-row Box-row--focus-gray py-2 d-flex position-relative js-navigation-item ghsc-header";
      $target.innerHTML = `
  ...
  function movePaginate(wrapper) {
    const pager =
      wrapper && $(".paginate-container, .ajax-pagination-form", wrapper);
    if (pager) {
