...
(async () => {
  ...
  window.config = { bgAnimationsDisabled: false };
  ...
  const xhr =
    (typeof GM != "undefined" && GM.xmlHttpRequest) || GM_xmlhttpRequest;
  ...
  const app = {
    ...
    urls: {
      update:
        "https://raw.githubusercontent.com/adamlui/github-star-history/main/greasemonkey/github-star-history.user.js",
    },
  };
  app.urls.resourceHost = `https://cdn.jsdelivr.net/gh/adamlui/github-star-history@${app.latestResourceCommitHash}`;
  const remoteAppData = await new Promise((resolve) =>
    xhr({
      method: "GET",
      url: `${app.urls.resourceHost}/assets/data/app.json`,
      onload: (resp) => resolve(JSON.parse(resp.responseText)),
    }),
  );
  Object.assign(app, {
    ...remoteAppData,
    urls: { ...app.urls, ...remoteAppData.urls },
  });
  ...
  function ghAlert(title, msg, btns, checkbox, width) {
    ...
    const handlers = {
      dismiss: {
        click(event) {
          if (
            ...
            event.target.closest("[class*=-close-btn]")
          )
            dismissAlert();
        },
        ...
        key(event) {
          if (
            !/^(?: |Space|Enter|Return|Esc)/.test(event.key) ||
            ![32, 13, 27].includes(event.keyCode)
          )
            return;
          for (const alertId of alertQueue) {
            // look to handle only if triggering alert is active
            const alert = document.getElementById(alertId);
            if (!alert || alert.style.display == "none") return;
            if (event.key.startsWith("Esc") || event.keyCode == 27)
              dismissAlert(); // and do nothing
            else {
              // Space/Enter pressed
      ...
      drag: {
        mousedown(event) {
          // find modal, update styles, attach listeners, init XY offsets
        ...
        mouseup() {
          // restore styles/pointer events, remove listeners, reset env.draggingModal
          Object.assign(env.draggingModal.style, {
            // restore styles
            cursor: "inherit",
            transition: "inherit",
    ...
    if (
      !modalStyle ||
      parseInt(modalStyle.getAttribute("last-updated"), 10) < thisUpdated
    ) {
      // if missing or outdated
      if (!modalStyle) {
        // outright missing, create/id/attr/append it first
        modalStyle = document.createElement("style");
        modalStyle.id = "chatgpt-modal-style";
    ...
    if (btns) {
      ...
      btns.forEach((buttonFn) => {
        // create title-cased labels + attach listeners
    ...
    if (checkbox) {
      ...
      checkboxLabel.textContent =
        checkboxFn.name[0].toUpperCase() + // capitalize first char
        checkboxFn.name
          .slice(1) // format remaining chars
    ...
    closeBtn.classList.add("modal-close-btn", "no-mobile-tap-outline");
    ...
    const closeSVGpath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path",
    );
    ...
    // Add listeners
    [modalContainer, closeBtn, closeSVG, dismissBtn].forEach(
      (elem) => (elem.onclick = handlers.dismiss.click),
    );
      ...
      if (childNode.nodeType == Node.TEXT_NODE) {
        ...
        if (elems.length > 0) {
          const elem = elems[0],
            ...
            tagNode = document.createElement(tagName);
          ...
          const beforeTextNode = document.createTextNode(
              text.substring(0, elem.index),
            ),
            afterTextNode = document.createTextNode(
              text.substring(elem.index + tagContent.length),
            );
  ...
  function safeWinOpen(url) {
    open(url, "_blank", "noopener");
  } // to prevent backdoor vulnerabilities
  ...
  function updateCheck() {
    ...
    xhr({
      method: "GET",
      url: app.urls.update + "?t=" + Date.now(),
      ...
      onload: (response) => {
        ...
        if (app.latestVer)
          for (let i = 0; i < 4; i++) {
            ...
            if (currentSubVer > latestSubVer)
            ...
            else if (latestSubVer > currentSubVer) {
              // if outdated
              ...
              ghAlert(
                ...
                function update() {
                  // button
                  GM_openInTab(
                    app.urls.update.replace("meta.js", "user.js") +
                      "?t=" +
                      Date.now(),
  ...
  async function insertStarHistory() {
    ...
    try {
      // to load/insert star history chart

      // Fetch image as blob
      const imgURL = sanitizeImgURL(
  ...
  function insertPhoneBtn(imgDataURL) {
    ...
    const repoBtnsDiv = document.querySelector(
      "#responsive-meta-container .d-flex.gap-2.mt-n3.mb-3.flex-wrap",
    );
    ...
    Object.assign(innerBtnSpan, {
      className: "Button-label tooltipped tooltipped-s",
      ariaLabel: "Star History",
    });
    ...
    innerBtnSpan.append(btnSVG);
    outerBtnSpan.append(innerBtnSpan);
  ...
  GM_registerMenuCommand(`💡 About ${app.name}`, async () => {
    ...
    const pStyle = "font-size: 1rem ; position: relative ; left: 3px ;",
      pBrStyle =
        "font-size: 1rem ; position: relative ; left: 9px ; bottom: 3px ;";
    const aboutAlertID = ghAlert(
      ...
      [
        ...
        function getSupport() {
          safeWinOpen(app.urls.support);
        },
        function discuss() {
          safeWinOpen(app.urls.discuss);
        },
      ],
      null,
      501,
  ...
  ["rpg", "rpw"].forEach(
    (
      cssType, // rising particles
    ) =>
      document.head.append(
        dom.create.style(GM_getResourceText(`${cssType}CSS`)),
      ),
  );
